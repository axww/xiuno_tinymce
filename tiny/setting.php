...
<div>/*链接加下划线*/</div>
<div>.message a{text-decoration:underline !important;}</div>
<div>/*表格简易样式*/</div>
<div>.message table,.message th,.message td{border:1px solid #999 !important;}</div>
<div>/*媒体样式优化*/</div>
<div>.message img{height:auto !important;}</div>
<div>.message img,.message audio,.message video{margin-top:8px !important;max-width:100% !important;}</div>
<div>@media(min-width:1000px){.message img,.message audio,.message video{max-width:720px !important;}}</div>
<hr />
<div><b>插件配置</b></div>
